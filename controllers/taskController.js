...
const getTask = asyncHandler(async (req, res) => {
  try {
    ...
    const formattedScreenshots = task.screenshots.map((screenshot) => ({
      imageUrl: `data:image/png;base64,${screenshot.data.toString("base64")}`,
      username: screenshot.username || "Unknown User",
    ...
    res.status(200).json({
      ...
      task: {
        ...task._doc,
        screenshots: formattedScreenshots, // Include formatted screenshots
      },
    });
  } catch (error) {
    console.log(error);
...
const upload_screenshot = asyncHandler(async (req, res) => {
  try {
    ...
    const { userId } = req.user; // Get user ID from the request
    ...
    task.screenshots.push({
      data: buffer,
      uploadedAt: new Date(),
      username: user?.email || "Unknown User", // Use user email or fallback to "Unknown User"
    });
